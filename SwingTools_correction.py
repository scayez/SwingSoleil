...
def integrate(image,params, maskfile=None ):
    ...
    i_iso = i_iso/(time*mi8*1e6)##############################################################chang value 30700????????????????????????????
...
def save_images(data_dir,file, integration,eiger_mean, basler_image,integration_dir, eiger_dir,basler_dir, sample_name=None):
    ...
    file_num = file.split('rodriguez_')[1]
    file_num = file_num.split('_2024')[0]
####################MArk on Basler image to adjust##########################
#donnés par Thomas: les coordonnées du faisceau :(508,534) - [60,40]
    basler_X0 = 508
    basler_Z0 = 534
    figsize = (8,8)
    ...
    fig1.suptitle(title)
    print(eiger_dir+'/'+'@eiger@'+sample_name+'@'+file_num+'.png')
    print(basler_dir+'/'+'@basler@'+sample_name+'@'+file_num+'.png' ) 
...
##################################################    
        ...
        ax[channel_index].set_xticks(np.arange(x_min, x_max + 1, 1))  # Set step of 1 for x-axis ticks
  

        # Note: You need to check which channel you're plotting for
        channel_1_positions = asso[asso['Channel'] == 1]['Position']
        channel_2_positions = asso[asso['Channel'] == 2]['Position']
        ...
        if channel_index == 0:  # Channel 1
            ax[0].scatter(position_index, 0, marker='o', edgecolors='red', facecolors='none', s=1000,linewidth=4)#, zorder=2)
            ax[0].scatter( channel_1_positions, np.zeros(len(channel_1_positions)), 
                        marker='+', color='yellow', s=100)#, linewidth=4,zorder=2)
            ax[1].scatter( channel_2_positions, np.zeros(len(channel_2_positions)), 
                  marker='+', color='yellow', s=100)#, zorder=2)
        else:  # Channel 2
          
            ax[1].scatter(position_index, 0, marker='o', edgecolors='red', facecolors='none', s=1000,linewidth=4)#, zorder=2)
            ax[1].scatter( channel_2_positions, np.zeros(len(channel_2_positions)), 
                  marker='+', color='yellow', s=100)#, zorder=2)
            ax[0].scatter( channel_1_positions, np.zeros(len(channel_1_positions)), 
                        marker='+', color='yellow', s=100)#, zorder=2)
